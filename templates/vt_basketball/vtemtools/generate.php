			...
			case 'widget-slideshow' :
				...
				case 'widget-carousel' :
					$extenItem = ($widgetName['content']['blocktitle'] != '' ? '<h3 class="widget-carousel-title clearfix">'.$widgetName['content']['blocktitle'].'</h3>' : '').'
									<div id="carousel'.($widgetName['content']['blockdata'] != 'k2' ? $widgetName['blockid'] : 'k2'.$widgetName['blockid']).'" class="vtem_main_carousel carousel clearfix">';
									if($widgetName['content']['blockdata'] == 'joomla') {
										$countItem =count(self::getList($widgetName));
										$extenItem .= '<div class="carousel-item">';
										foreach (self::getList($widgetName) as $key => $list) :								
											$extenItem .= '<div class="carousel-item-inside clearfix '.$widgetName['options']['itemanimation'].'" data-wow-delay="0.'.($key+2).'s">'.$list->displayImage.
											...
											if ((($key + 1) % $widgetName['options']['blockitemrow'] == 0) && ($key + 1) < $countItem)
												$extenItem .= '</div><div class="carousel-item">';
										endforeach;
										$extenItem .= '</div>';
									} elseif ($widgetName['content']['blockdata'] == 'k2') {
										$doc = JFactory::getDocument();
										$renderer = $doc->loadRenderer('module');
										$module = JModuleHelper::getModule('mod_k2_content');
										$attribs['style'] = 'none';
										$module = clone $module;
										$module->id = $widgetName['blockid'];
										$module->params = '{"moduleclass_sfx":" vtemk2box'.$widgetName['blockid'].'","getTemplate":"Default","source":"filter","catfilter":"1","category_id":["'.$widgetName['content']['k2catid'].'"],"getChildren":"'.$widgetName['content']['k2getChildren'].'","itemCount":"'.$widgetName['content']['k2itemCount'].'","itemsOrdering":"'.$widgetName['content']['k2itemsOrdering'].'","FeaturedItems":"'.$widgetName['content']['k2FeaturedItems'].'","popularityRange":"","videosOnly":"0","item":"0","itemTitle":"'.$widgetName['content']['k2itemTitle'].'","itemAuthor":"0","itemAuthorAvatar":"0","itemAuthorAvatarWidthSelect":"custom","itemAuthorAvatarWidth":"50","userDescription":"0","itemIntroText":"'.$widgetName['content']['k2itemIntroText'].'","itemIntroTextWordLimit":"'.$widgetName['content']['k2itemIntroLimit'].'","itemImage":"'.$widgetName['content']['k2itemImage'].'","itemImgSize":"'.$widgetName['content']['k2itemImgSize'].'","itemVideo":"0","itemVideoCaption":"0","itemVideoCredits":"0","itemAttachments":"0","itemTags":"'.$widgetName['content']['k2itemTags'].'","itemCategory":"'.$widgetName['content']['k2itemCategory'].'","itemDateCreated":"'.$widgetName['content']['k2itemDateCreated'].'","itemHits":"'.$widgetName['content']['k2itemHits'].'","itemReadMore":"'.$widgetName['content']['k2itemReadMore'].'","itemExtraFields":"'.$widgetName['content']['k2itemExtraFields'].'","itemCommentsCounter":"0","feed":"0","itemPreText":"","itemCustomLink":"0","itemCustomLinkTitle":"","itemCustomLinkURL":"","itemCustomLinkMenuItem":"101","K2Plugins":"1","JPlugins":"1","cache":"1","cache_time":"900","module_tag":"div","bootstrap_size":"0","header_tag":"h3","header_class":"","style":"0"}';
										$extenItem .= $renderer->render($module, $attribs);
									} else {
										$countItem =count($widgetName['content']['itemList']);
										$extenItem .= '<div class="carousel-item">';
										foreach ($widgetName['content']['itemList'] as $key => $list) :
											$extenItem .='<div class="carousel-item-inside '.$widgetName['options']['itemanimation'].' clearfix" data-wow-delay="0.'.($key+2).'s">
							...
							$extenItem .='</div>
										<script type="text/javascript">
											jQuery(document).ready(function(){
												jQuery("#carousel'.$widgetName['blockid'].', .vtemk2box'.$widgetName['blockid'].' > ul").owlCarousel({
				...
				case 'widget-gmap' :
					...
					$doc->addScript("https:////maps.google.com/maps/api/js?key=AIzaSyAqKas9IeeE30INII-BgS-iqlqldhBoc2E");
					...
					$extenItem = ($widgetName['content']['blocktitle'] != '' ? '<h3 class="widget-gallery-title clearfix">'.$widgetName['content']['blocktitle'].'</h3>' : '');
									$extenItem .= '<div class="galleryType'.$widgetName['options']['blockfilter'].'"><div id="gallery'.$widgetName['blockid'].'" class="widget-gallery-content vtem-gallery clearfix" style="margin:-'.$widgetName['options']['blockspacing'].'">';
										foreach ($widgetName['content']['itemList'] as $key => $list) :
											...
											$extenItem .= '<div class="vtem-gallery-item clearfix" data-item-tags="'.trim($list['addontag']).'" style="padding:'.$widgetName['options']['blockspacing'].'">
									...
									if ($widgetName['options']['blockfilter'] == 'nav') {
										$extenItem .="<script type='text/javascript'>jQuery(window).load(function () {
											jQuery('#gallery".$widgetName['blockid']."').tagFillter();
										});</script>";
									}
