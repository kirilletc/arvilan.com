...
$script[] = 'tinymce.init({
			...
			external_plugins: {"fontawesome": "'.JURI::root().'templates/'.$this->form->getValue('template').'/vtemtools/extends/font-awesome/tinymce/plugin.min.js"},
			content_css: "'.JURI::root().'templates/'.$this->form->getValue('template').'/vtemtools/extends/font-awesome/css/font-awesome.min.css",
			plugins : "autolink,lists,image,charmap,print,preview,anchor,pagebreak,code,save,textcolor,colorpicker,importcss,searchreplace,insertdatetime,link,fullscreen,table,emoticons,media,hr,directionality,paste,visualchars,visualblocks,nonbreaking,template,print,wordcount,advlist,autosave,contextmenu,fontawesome noneditable",
			// Toolbar
			toolbar1: "bold italic underline strikethrough | alignleft aligncenter alignright alignjustify searchreplace | bullist numlist | outdent indent | undo redo | link unlink anchor image | forecolor,backcolor | fullscreen table | subscript superscript charmap emoticons media hr | ltr rtl inserttime cut copy paste | visualchars visualblocks nonbreaking blockquote template | print preview | code | fontawesome"
		});';
